...
<script setup lang="ts">
    ...
    const activePreset = computed(() => styles.find((p) => p.id === store.state.activePresetId));

    const activeAmbiance = computed(() => activePreset.value?.ambiances?.find((ambiance) => ambiance.id === store.state.activeAmbianceId));
    ...
    function instrumentImage(instrumentId: string): string {
        return instrumentConfig(instrumentId)?.image ?? '';
    }
